...
group_percent <- function(data){
  ...
  percent_group<- sum_data %>% mutate('Total' = rowSums(select(.,-weeks))) %>% mutate(across(c(3:ncol(sum_data)-1), .fns = ~./Total*100)) %>% select(-Total)
  
  return(percent_group)
}

getMean2percent <- function(data){
  bacteria_group_2 <- as.data.frame(list(colMeans(data[,-1])), col.names = 'Mean') %>% rownames_to_column(var = 'Bacteria') %>% 
